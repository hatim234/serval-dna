...
/* Release all the identities that have a given PIN, by unlinking them from the in-memory cache
 * list.  DOES call keyring_free_identity(id) on each released identity.
 */
...
/* Release the given identity by unlinking it from the in-memory cache list.  Mark all other
 * identities that have the same PIN as having a released identity, so that if the PIN is
 * re-entered, it will re-try the decryption.  Does NOT call keyring_free_identity(id), so the
 * identity's contents remain intact; the caller must free the identity if desired.
 */
void keyring_release_identity(keyring_file *k, keyring_identity *id)
{
  ...
  for (i = &k->identities; *i; i = &(*i)->next) {
    ...
    if (iid == id)
      prev = i;
    // mark any other identities that have the same PIN that the PIN has at least one released
    // identity
    if (id->PKRPin && iid->PKRPin && strcmp(id->PKRPin, iid->PKRPin) == 0)
      iid->PKRPin_released = 1;
  }
...
/* Release the identity with the given SID, by unlinking it from the in-memory cache list.  Mark all
 * other identities that have the same PIN as having a released identity, so that if the PIN is
 * re-entered, it will re-try the decryption.  Return zero if an identity was found and released,
 * -1 if no such identity was found.  DOES call keyring_free_identity(id) on any released identity
 * before returning.
 */
int keyring_release_subscriber(keyring_file *k, const sid_t *sid)
{
  ...
  keyring_identity **i;
  for (i = &k->identities; *i; i = &(*i)->next) {
    if (cmp_sid_t((*i)->box_pk, sid) == 0) {
      keyring_release_identity(k, *i);
      keyring_free_identity(*i);
      return 0;
    }
  }
...
unsigned keyring_enter_pin(keyring_file *k, const char *pin)
{
  ...
  // check if PIN is already entered and no identity with that PIN has been released
  {
    keyring_identity *id;
    for (id = k->identities; id; id = id->next) {
      if (*pin ? (id->PKRPin && strcmp(id->PKRPin, pin) == 0) : (!id->PKRPin)) {
	++identity_count;
	if (id->PKRPin_released)
	  released = 1;
      }
    }
  }

  // try to decrypt if there are no identities already open with the given PIN, or if the PIN has
  // been marked as having at least one identity that has been released
  if (!identity_count || released) {
    unsigned slot;
    ...
    // now all identities with the given PIN have been unlocked, so mark the PIN as not having any
    // released identities
    if (released) {
      keyring_identity *id;
      for (id = k->identities; id; id = id->next) {
	if (*pin ? (id->PKRPin && strcmp(id->PKRPin, pin) == 0) : (!id->PKRPin)) {
	  id->PKRPin_released = 0;
	}
      }
    }
